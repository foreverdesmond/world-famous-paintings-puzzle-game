...
export function App() {
  ...
  const frameOrientation = getArtworkOrientation(session.artwork.width, session.artwork.height) === 'portrait' ? 'portrait' : 'landscape'
  return <main className={`app-shell game-page${isPreview ? ' is-preview' : ''}`}>
    ...
    <div className="game-layout"><section className="board-panel" aria-label="拼图区域"><div className={`puzzle-column puzzle-column--${frameOrientation}`}>
      <PuzzleFrame orientation={frameOrientation}>
        {isPreview ? <>
          <div className="preview-note">原图预览 · 5 秒</div>
          <PreviewArtwork artwork={session.artwork} />
        </> : <PuzzleBoard board={session.board} artwork={session.artwork} frameOrientation={frameOrientation} selectedTileIndex={session.selectedTileIndex} celebrating={isCelebrating} disabled={game.status !== 'playing'} onTileClick={game.clickTile} />}
      </PuzzleFrame>
      {resultStatus && <ResultDialog status={resultStatus} elapsedMs={game.elapsedMs} onNextStage={game.nextStage} onExit={game.exitGame} />}
    </div></section><GameSidebar stage={session.stage} level={grid.level} artwork={session.artwork} elapsedMs={game.elapsedMs} /></div>
  </main>
}
